Windows及Linux下的Dns嗅探器：在相应环境进入run.py所在文件下（windows直接双击run.py,Linux输入命令python run.py)，运行后根据命令行提示选择网卡，之后就可以在当前目录下看到新生产的DNS域名文件dns.txt。
...
注意：Winpcap和Libpcap这两个是必须安装的，不然既无法运行而且也无法编译源码生成可执行文件，python需要2.x版本，3.x版本运行run.py会报错，需要的可直接修改run.py（很简单的）。Windows下电脑电源设置为从不休眠，因为休眠后会关闭嗅探器。
